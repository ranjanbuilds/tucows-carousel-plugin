...
import {ViewModel, SWIPE_OBJECT_DEFAULT} from './ting-carousel';

import clone from 'steal-clone';

// ViewModel unit tests
describe('util/ting-carousel', () => {
	describe('viewModel properties', () => {
		...
		describe('isDesktop', () => {
			// setup
			let isMobileStub = sinon.stub(); // set in clone

			clone({'ting-node/util/common/utils': {'isMobile': isMobileStub} }).import('./ting-carousel').then(
				({ViewModel}) => {
			...
			it('should call isMobile with isTablet', () => {
				// run
				vm.isDesktop;
				// test
				isMobileStub.calledOnce.should.be.true;
				isMobileStub.calledWith('isTablet').should.be.true;
			});

			it('should return the opposite of isMobile', () => {
				let boolean = chance.bool();
				// setup 
				isMobileStub.returns(boolean);
				// test
				vm.isDesktop.should.equal(!boolean);
			});
		});
